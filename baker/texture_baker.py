...
def prepare_selected_objects_for_id_pass(context, settings, shared_image, alpha_pass=False):
    ...
    alpha_pass = False → use prepare_material_for_id (RGB_ID → Emission Color)
    alpha_pass = True  → use prepare_material_for_id_alpha (Alpha_ID → Emission Color)
    """
    report_data = {"skipped": [], "processed": []}
    ...
    def execute(self, context):
        ...
        try:
            if settings.map_type != 'ID':
                ...
                # Pass 1: Color ID (RGB_ID → Emission Color) → _C_ID
                ...
                # Pass 2: Alpha ID (Alpha_ID → greyscale Emission Color) → _A_ID
...
# ---------------------------------------------------------------------------
# Panels – Baking (hooked under T8 Tools > Baking Tools)
# ---------------------------------------------------------------------------
