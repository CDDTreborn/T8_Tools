...
def _apply_group_interface_socket_defaults(node, node_def):
    """
    Restore exposed node group socket defaults on Group Input / Group Output nodes.
    This affects the values visible on the outside of the node group.
    """
    ...
    for idx, s_def in enumerate(socket_defs):
        if idx >= len(sockets):
            continue
        _set_default_value(sockets[idx], s_def.get("default_value"))
...
def rebuild_node_groups_from_template_data(template_data, prefix="REBUILT_"):
    ...
    for original_name, g_def in group_defs.items():
        ...
        for node_def in g_def.get("nodes", []):
            ...
            # Restore exposed node-group fallback/default values.
            _apply_group_interface_socket_defaults(node, node_def)
